...
- (void)createWorldLayer {
  ...
  // Parallaxing Nodes
  self.cloudSlow = [[PPCloudParallaxSlow alloc]
      initWithSize:CGSizeMake(self.size.width * 2, self.size.height)];
  self.cloudSlow.zPosition = SceneLayerClouds;
  [self.worldNode addChild:self.cloudSlow];

  self.cloudFast = [[PPCloudParallaxFast alloc]
      initWithSize:CGSizeMake(self.size.width * 2, self.size.height)];
  ...
  [self.snowEmitter
      setPosition:CGPointMake(self.size.width / 2, self.size.height / 2)];
  ...
  self.waterSurface =
      [PPWaterSprite surfaceWithStartPoint:surfaceStart
                                  endPoint:surfaceEnd
  ...
  SKEmitterNode *playerBubbleEmitter =
      [PPSharedAssets sharedBubbleEmitter].copy;
  ...
  _playerBubbleBirthrate =
      playerBubbleEmitter.particleBirthRate; // To reset the simulation
  ...
  [playerStarEmitter
      setPosition:CGPointMake(player.position.x - player.size.height / 2,
                              player.position.y)];
  [self.worldNode addChild:playerStarEmitter];

  _playerStarBirthrate =
      playerStarEmitter.particleBirthRate; // To reset the simulation
  [playerStarEmitter
      setParticleBirthRate:0]; // Start the emitter only during boost
  ...
  [self.coinSpawnPosition
      setPosition:CGPointMake(self.size.width * 2, self.size.height / 4)];
  [self.worldNode addChild:self.coinSpawnPosition];

  // Screen Physics Boundary
  SKSpriteNode *boundary = [SKSpriteNode
      spriteNodeWithColor:[SKColor clearColor]
                     size:CGSizeMake(self.size.width, self.size.height)];
  boundary.physicsBody =
      [SKPhysicsBody bodyWithEdgeLoopFromRect:boundary.frame];
...
- (void)createHudLayer {
  ...
  SSKButtonNode *pauseButton = [SSKButtonNode
      buttonWithTexture:[PPSharedAssets sharedButtonPause]
               idleSize:CGSizeMake(kButtonIdleWidth / 2, kButtonIdleWidth / 2)
           selectedSize:CGSizeMake(kButtonSelectedWidth / 2,
                                   kButtonSelectedWidth / 2)];
  [pauseButton
      setPosition:CGPointMake(self.size.width / 2 - pauseButton.size.width / 2 -
                                  padding,
                              -self.size.height / 2 +
                                  pauseButton.size.height / 2 + padding)];
  [pauseButton setTouchUpInsideTarget:self
                             selector:@selector(pauseButtonTouched)];
  [self.hudNode addChild:pauseButton];

  PPBoostMeter *boostMeter =
      [[PPBoostMeter alloc] initWithSize:CGSizeMake(10, 50)];
  [boostMeter setName:@"boostMeter"];
  [boostMeter
      setPosition:CGPointMake(-self.size.width / 2 + boostMeter.size.width / 2 +
                                  padding,
                              self.size.height / 2 -
                                  boostMeter.size.height / 2 - padding)];
  [self.hudNode addChild:boostMeter];

  NSString *fontName = @"AmericanTypewriter";
  CGFloat fontSize = 12.0;

  SSKScoreNode *scoreCounter =
      [SSKScoreNode scoreNodeWithFontNamed:fontName
                                  fontSize:20
  ...
  [scoreCounter setPosition:CGPointMake(boostMeter.position.x +
                                            boostMeter.size.width / 2 + padding,
  ...
  [coinNode setPosition:CGPointMake(self.size.width / 2 - coinNode.size.width,
                                    self.size.height / 2 -
                                        coinNode.size.height / 2 - padding)];
  [self.hudNode addChild:coinNode];

  SSKScoreNode *coinCounter =
      [SSKScoreNode scoreNodeWithFontNamed:fontName
                                  fontSize:fontSize
                                 fontColor:[SKColor whiteColor]];
  ...
  [coinCounter setPosition:CGPointMake(coinNode.position.x,
                                       coinNode.position.y -
                                           coinNode.size.height / 2 - padding)];
  [coinCounter
      setScore:[[PPUserManager sharedManager] getTotalCoins].integerValue];
  [self.hudNode addChild:coinCounter];
}

- (void)hudLayerFadeInAnimation {
  [self.hudNode
      runAction:[SKAction moveDistance:CGVectorMake(kMoveAndFadeDistance, 0)
                    fadeInWithDuration:kMoveAndFadeTime]];
}
...
- (void)hudLayerFadeOutAnimation {
  if (self.hudNode) {
    [self.hudNode
        runAction:[SKAction moveDistance:CGVectorMake(kMoveAndFadeDistance, 0)
                      fadeOutWithDuration:kMoveAndFadeTime]];
  }
}
...
- (void)createGameOverLayer {
  ...
  SKLabelNode *gameOverLabel =
      [self createNewLabelWithText:@"Game Over" withFontSize:55];
  ...
  NSString *currentScoreString =
      [(SSKScoreNode *)[self.hudNode childNodeWithName:@"scoreCounter"] text];

  // Score label
  SKLabelNode *scoreLabel = [self
      createNewLabelWithText:[NSString stringWithFormat:@"Score: %@",
                                                        currentScoreString]
                withFontSize:30];
  [scoreLabel setFontColor:[SKColor darkRed]];
  [scoreLabel setPosition:CGPointMake(gameOverLabel.position.x,
                                      gameOverLabel.position.y - 50)];
  [self.gameOverNode addChild:scoreLabel];

  // Coins Label
  NSUInteger totalCoinsCount =
      [(SSKScoreNode *)[self.hudNode childNodeWithName:@"coinCounter"] count];
  SKLabelNode *coinsLabel = [self
      createNewLabelWithText:[NSString stringWithFormat:@"Coins: %lu",
                                                        (unsigned long)
                                                            totalCoinsCount]
                withFontSize:30];
  [coinsLabel setFontColor:[SKColor darkRed]];
  [coinsLabel setPosition:CGPointMake(gameOverLabel.position.x,
...
- (void)gameOverFadeInAnimation {
  [self.gameOverNode
      runAction:[SKAction moveDistance:CGVectorMake(kMoveAndFadeLongDistance, 0)
                    fadeInWithDuration:kMoveAndFadeTime]];
}
...
- (void)gameStart {
  ...
  [self runAction:[SKAction fadeOutWithDuration:.5]
           onNode:[self childNodeWithName:@"menu"]];
...
- (void)startGameAnimations {
  [self runAction:[SKAction waitForDuration:.5]
       completion:^{
         ...
         SKAction *splashRight =
             [self waterSplashAtPosition:CGPointMake(self.size.width / 2, 0)];
         SKAction *wait = [SKAction waitForDuration:.5];
         [self runAction:[SKAction repeatActionForever:[SKAction sequence:@[
                                     splashRight,
                                     wait
                                   ]]]
                 withKey:@"idleSplashKey"];
...
- (void)resetGame {
  SKSpriteNode *fadeNode =
      [SKSpriteNode spriteNodeWithColor:[SKColor whiteColor] size:self.size];
  [fadeNode setZPosition:SceneLayerFadeOut];
  [fadeNode setAlpha:0];
  [self addChild:fadeNode];

  [fadeNode runAction:[SKAction fadeInWithDuration:.5]
           completion:^{
             ...
             [self runAction:[SKAction waitForDuration:.25]
                  completion:^{
                    [fadeNode runAction:[SKAction fadeOutWithDuration:.5]
                             completion:^{
...
- (void)menuButtonTouched {
  ...
  NSInteger currentScore =
      [(SSKScoreNode *)[self.hudNode childNodeWithName:@"scoreCounter"] count];

  NSString *welcomeText = [NSString
      stringWithFormat:@"%@ wants to know if you can beat this score: %li!",
                       username, (long)currentScore];

  NSURL *URL =
      [NSURL URLWithString:[NSString stringWithFormat:@"race?color=blue"]];
  NSDictionary *data = @{ @"welcomeText" : welcomeText };
...
- (PPButtonNode *)retryButton {
  PPButtonNode *retryButton = [PPButtonNode
      buttonWithTexture:[PPSharedAssets sharedButtonPlay]
               idleSize:CGSizeMake(kButtonIdleWidth, kButtonIdleWidth)
           selectedSize:CGSizeMake(kButtonSelectedWidth, kButtonSelectedWidth)];
...
- (void)setPauseOnAllParallaxNodes:(BOOL)shouldPause {
  if (shouldPause) {
    ...
    [self.cloudFast.parallaxLayer
        setMoveSpeedMultiplier:_currentParallaxMultiplier];
    [self.cloudSlow.parallaxLayer
        setMoveSpeedMultiplier:_currentParallaxMultiplier];
  }
}
...
- (SKSpriteNode *)createPauseLayer {
  SKSpriteNode *dimSprite =
      [SKSpriteNode spriteNodeWithColor:[SKColor blackColor] size:self.size];
...
- (void)startCoinSpawnIntervals {
  ...
  SKAction *getSpawnPosition = [SKAction runBlock:^{
    CGFloat randomHeight =
        SSKRandomFloatInRange(self.size.height / 5, self.size.height);
  ...
  SKAction *spawnIntervals =
      [SKAction sequence:@[ getSpawnPosition, spawnSequence, wait ]];

  [self runAction:[SKAction repeatActionForever:spawnIntervals]
          withKey:kCoinSpawnKey];
}

- (void)stopCoinSpawn {
  [self removeActionForKey:kCoinSpawnKey];
  [self.worldNode enumerateChildNodesWithName:kCoinName
...
- (void)checkCoinsDidIntersect {
  [self.worldNode
      enumerateChildNodesWithName:kCoinName
                       usingBlock:^(SKNode *node, BOOL *stop) {
                         PPCoinNode *coin = (PPCoinNode *)node;
                         if (CGRectIntersectsRect([self currentPlayer].frame,
                                                  coin.frame)) {
                           [coin removeFromParent];
                           [coin setPosition:[self.worldNode
                                                 convertPoint:[[self
                                                                  currentPlayer]
                                                                  position]
                                                       toNode:self]];
                           ...
                           [self popCircleOnNode:[self currentPlayer]
                                           color:[SKColor colorWithRed:0.935
                                                                 green:0.744
                                                                  blue:0.042
                                                                 alpha:1.000]];

                           [coin runAction:[SKAction moveTo:[self getScoreCoin]
                                                                .position
                                                   duration:0.3]
                                completion:^{
                                  [self scoreCoinPop];
                                  [coin removeFromParent];
                                  [(SSKScoreNode *)[self.hudNode
                                      childNodeWithName:@"coinCounter"]
                                      increment];
                                }];
                         }
                       }];
...
- (void)saveCoins {
  ...
  [[PPUserManager sharedManager]
      saveCoins:[NSNumber numberWithInteger:coinCount]];
...
- (void)spawnCoinAtPosition:(CGPoint)position {
  ...
  [self runAction:[SKAction runBlock:^{
          ...
          [coin runAction:[SKAction moveToX:-self.size.width / 4 * 3
                                   duration:kParallaxNormalMoveSpeed]
                  withKey:kCoinMoveKey
               completion:^{
                 [coin removeFromParent];
               }];
        }]];
...
- (SKAction *)spawnCoinsWithCount:(NSUInteger)count
...
- (void)spawnFishForever {
  if ([self actionForKey:@"fishSpawn"])
    return;
  ...
  SKAction *spawnAndMove = [SKAction runBlock:^{
    ...
    CGFloat randY =
        SSKRandomFloatInRange(self.size.height / 10, self.size.height / 2);

    // Spawn a new fish
    PPFishNode *fish =
        [[PPFishNode alloc] initWithType:SSKRandomFloatInRange(0, 4)];
...
- (void)checkFishDidIntersect {
  [self.worldNode
      enumerateChildNodesWithName:kFishName
                       usingBlock:^(SKNode *node, BOOL *stop) {
...
- (void)popCircleOnNode:(SKSpriteNode *)node color:(SKColor *)color {
  PPCircleNode *circle =
      [PPCircleNode shapeNodeWithCircleOfRadius:node.size.width / 4];
...
- (PPPlayer *)blackPenguin {
  return [self penguinWithType:PlayerTypeBlack
                         atlas:[PPSharedAssets sharedPenguinBlackTextures]];
}
...
- (void)clampPlayerVelocity {
  ...
  if (player.physicsBody.velocity.dy >= kPlayerUpperVelocityLimit) {
    [player.physicsBody setVelocity:CGVectorMake(player.physicsBody.velocity.dx,
                                                 kPlayerUpperVelocityLimit)];
  }

  if (player.position.y >
      [self.worldNode childNodeWithName:@"water"].position.y) {
    if (player.physicsBody.velocity.dy <= kPlayerLowerAirVelocityLimit) {
      [player.physicsBody
          setVelocity:CGVectorMake(player.physicsBody.velocity.dx,
                                   kPlayerLowerAirVelocityLimit)];
    }
  } else {
    if (player.physicsBody.velocity.dy <= kPlayerLowerWaterVelocityLimit) {
      [player.physicsBody
          setVelocity:CGVectorMake(player.physicsBody.velocity.dx,
                                   kPlayerLowerWaterVelocityLimit)];
    }
  }
}
...
- (void)trackPlayerForSplash {
  ...
  if (_lastPlayerHeight < 0 && newPlayerHeight > 0) {
    ...
    [self.waterSurface splash:[self currentPlayer].position
                        speed:splashStrength];
    [self runOneShotEmitter:self.splashUpEmitter
                   location:[self currentPlayer].position];
  ...
  // Cross surface from top
    ...
    [self.waterSurface splash:[self currentPlayer].position
                        speed:splashStrength];
    [self runOneShotEmitter:self.splashDownEmitter
                   location:[self currentPlayer].position];
...
- (void)updateStarEmitterPosition {
  ...
  SKEmitterNode *starEmitter =
      (SKEmitterNode *)[self.worldNode childNodeWithName:@"starEmitter"];
  [starEmitter
      setPosition:CGPointMake(starEmitter.position.x, playerPosition.y)];
}
...
- (SKAction *)runStarExplosionAtPosition:(CGPoint)position {
  return [SKAction runBlock:^{
    [self runOneShotEmitter:[PPSharedAssets sharedStarExplosionEmitter]
                   location:position];
  }];
}
...
- (PPIcebergObstacle *)newIceBerg {
  ...
  PPIcebergObstacle *obstacle =
      [PPIcebergObstacle icebergWithType:IceBergTypeNormal];
  [obstacle setName:kObstacleName];
  [obstacle setScale:newScale];
  [obstacle
      setPosition:CGPointMake(self.size.width * 2, obstacle.size.height / 10)];
...
- (void)startObstacleSpawnSequence {
  ...
  SKAction *spawnFloatMove = [SKAction runBlock:^{
    ...
    [obstacle runAction:[SKAction moveToX:-self.size.width / 4 * 3
                                 duration:kParallaxNormalMoveSpeed]
                withKey:kObstacleMoveKey
             completion:^{
...
- (void)stopObstacleMovement {
  [self.worldNode enumerateChildNodesWithName:@"obstacle"
                                   usingBlock:^(SKNode *node, BOOL *stop) {
...
- (void)startScoreCounter {
  ...
  SKAction *incrementScore = [SKAction runBlock:^{
    [(SSKScoreNode *)[self.hudNode childNodeWithName:@"scoreCounter"]
        increment];
  }];
...
- (void)checkIfHighScore {
  NSInteger currentScore =
      [(SSKScoreNode *)[self.hudNode childNodeWithName:@"scoreCounter"] count];
  NSInteger highScore =
      [[PPUserManager sharedManager] getHighScore].integerValue;

  // Add Custom Sharing Text WITH Current Score
  NSString *customText =
      [NSString stringWithFormat:@"Try to beat my #megacool score: %li",
                                 (long)currentScore];
  [[Megacool sharedMegacool] setSharingText:customText];

  if (currentScore > highScore) {
    [[PPUserManager sharedManager]
        saveHighScore:[NSNumber numberWithInteger:currentScore]];
...
- (void)highScoreAnimation {
  ...
  SKLabelNode *highScoreLabel =
      [self createNewLabelWithText:@"Highscore!" withFontSize:30];
  [highScoreLabel setFontColor:[SKColor whiteColor]];
  [highScoreLabel setZPosition:SceneLayerGameOver];
  [highScoreLabel
      setPosition:CGPointMake(0, self.size.height / 2 -
                                     highScoreLabel.frame.size.height)];
  ...
  SKAction *explosion =
      [self runStarExplosionAtPosition:highScoreLabel.position];
  ...
  SKAction *pulsate = [SKAction
      repeatActionForever:[SKAction sequence:@[ idleGrow, idleShrink ]]];

  [highScoreLabel
       runAction:[SKAction sequence:@[ wait, explosion, grow, shrink ]]
      completion:^{
...
- (void)updateGravity {
  if ([self currentPlayer].position.y >
      [self.worldNode childNodeWithName:@"water"].position.y) {
...
- (void)runColorChangeOnLabel:(SKLabelNode *)label interval:(CGFloat)interval {
  ...
  SKAction *sequence = [SKAction repeatActionForever:[SKAction sequence:@[
                                   changeToRed,
                                   wait,
                                   changeToBlue,
                                   wait
...
- (SKLabelNode *)createNewLabelWithText:(NSString *)text
  ...
  SKLabelNode *label =
      [SKLabelNode labelNodeWithFontNamed:@"AmericanTypewriter"];
...
- (void)removeUnparentedNodes {
  ...
  [self enumerateChildNodesWithName:kRemoveName
                         usingBlock:^(SKNode *node, BOOL *stop) {
                           [node removeFromParent];
                         }];
...
- (void)resolveCollisionFromFirstBody:(SKPhysicsBody *)firstBody
                           secondBody:(SKPhysicsBody *)secondBody {
  if (self.gameState == Playing) {
    [self.worldNode runAction:[SKAction shakeWithDuration:.1
                                                amplitude:CGPointMake(10, 10)]];
    [self gameEnd];
  }
}
...
- (void)boostActionGroup {
  if ([self actionForKey:@"boostKey"])
    return;
  if ([self currentBoostMeter].currentProgress < 0.49f)
    return;
  ...
  SKAction *starExplosion =
      [self runStarExplosionAtPosition:[self currentPlayer].position];
...
- (SKAction *)adjustBoostSpeed:(NSUInteger)speed {
  return [SKAction runBlock:^{
    ...
    // Spawning nodes
    [self setNodeSpeed:speed
          withNodeName:kCoinName
        withActionName:kCoinMoveKey];
    [self setNodeSpeed:speed
          withNodeName:kObstacleName
        withActionName:kObstacleMoveKey];
    [self setNodeSpeed:speed
          withNodeName:kFishName
        withActionName:kFishMoveKey];
...
- (void)setNodeSpeed:(CGFloat)speed
        withNodeName:(NSString *)nodeName
      withActionName:(NSString *)actionName {
  [self.worldNode enumerateChildNodesWithName:nodeName
                                   usingBlock:^(SKNode *node, BOOL *stop) {
                                     SKAction *moveAction =
                                         [node actionForKey:actionName];
...
- (void)loadMenuScene {
  [PPMenuScene loadSceneAssetsWithCompletionHandler:^{
    ...
    SKTransition *fade =
        [SKTransition fadeWithColor:[SKColor whiteColor] duration:1];
...
- (void)setNewWorldPositionWithOffset:(CGVector)amountToOffset {
  [self.worldNode
      setPosition:CGPointMake(amountToOffset.dx, amountToOffset.dy)];
}

- (CGVector)amountToOffsetTop {
  return CGVectorMake(
      -([self distanceFromCenterToTopRight].dx *
        [self percentageOfMaxScaleWithRatio:[self topZoomRatio]]),
      -([self distanceFromCenterToTopRight].dy *
        [self percentageOfMaxScaleWithRatio:[self topZoomRatio]]));
}
...
- (CGVector)amountToOffsetBottom {
  return CGVectorMake(
      -([self distanceFromCenterToTopRight].dx *
        [self percentageOfMaxScaleWithRatio:[self bottomZoomRatio]]),
      [self distanceFromCenterToTopRight].dy *
          [self percentageOfMaxScaleWithRatio:[self bottomZoomRatio]]);
}

- (CGFloat)topZoomRatio {
  CGFloat ratioDistanceFromTop =
      fabs(([self currentPlayerDistanceFromTopBoundary] /
            [self distanceFromBoundaryToMaxZoomBoundary]));
...
- (CGFloat)bottomZoomRatio {
  CGFloat ratioDistanceFromBottom =
      fabs(([self currentPlayerDistanceFromBottomBoundary] /
            [self distanceFromBoundaryToMaxZoomBoundary]));
...
- (CGVector)distanceFromCenterToTopRight {
  return SSKDistanceBetweenPoints(
      CGPointZero, CGPointMake(self.size.width / 2, self.size.height / 2));
}
